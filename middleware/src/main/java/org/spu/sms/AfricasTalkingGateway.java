...
public class AfricasTalkingGateway {
    ...
    public JSONArray fetchMessages(int lastReceivedId_) throws Exception {
        String requestUrl = getSmsUrl() + "?" + URLEncoder.encode("username", "UTF-8") + "="
                + URLEncoder.encode(_username, "UTF-8") + "&" + URLEncoder.encode("lastReceivedId", "UTF-8") + "="
                + URLEncoder.encode(Integer.toString(lastReceivedId_), "UTF-8");
    ...
    public JSONArray fetchPremiumSubscriptions(String shortCode_, String keyword_, int lastReceivedId_)
            throws Exception {
        ...
        String requestUrl = getSubscriptionUrl() + "?username=" + _username + "&shortCode=" + shortCode_ + "&keyword="
                + keyword_ + "&lastReceivedId=" + lastReceivedId_;
    ...
    public JSONObject initiateMobilePaymentCheckout(String productName_, String phoneNumber_, String currencyCode_,
            Double amount_, Map<String, String> metadata_) throws Exception {
        JSONObject requestBody = new JSONObject().put("username", _username).put("productName", productName_)
                .put("phoneNumber", phoneNumber_).put("currencyCode", currencyCode_).put("amount", amount_)
                .put("metadata", metadata_);
        String response = sendJsonPOSTRequest(requestBody.toString(), getMobilePaymentCheckoutUrl());
        return new JSONObject(response);
    }

    public JSONArray mobilePaymentB2CRequest(String productName_, List<MobilePaymentB2CRecipient> recipients_)
            throws Exception {
        ...
        if (DEBUG)
            System.out.println("Raw Request: " + requestBody.toString());
    ...
    private String getApiHost() {
        return (_environment == "sandbox") ? "https://api.sandbox.africastalking.com"
                : "https://api.africastalking.com";
    }

    private String getPaymentHost() {
        return (_environment == "sandbox") ? "https://payments.sandbox.africastalking.com"
                : "https://payments.africastalking.com";
    }

    private String getVoiceHost() {
        return (_environment == "sandbox") ? "https://voice.sandbox.africastalking.com"
...
class MobilePaymentB2CRecipient {
    ...
    public MobilePaymentB2CRecipient(String phoneNumber_, String currencyCode_, Double amount_) {
    ...
    public JSONObject toJSON() throws Exception {
        return new JSONObject().put("phoneNumber", _phoneNumber).put("currencyCode", _currencyCode)
                .put("amount", _amount).put("metadata", _metadata);
    }
}
